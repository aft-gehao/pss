...
function OnRightClick(event, treeId, treeNode) {
    ...
    if (!treeNode && event.target.tagName.toLowerCase() != "button" && $(event.target).parents("a").length == 0) {
        showRMenu("root", event.clientX, event.clientY,treeNode.id,treeNode.name,treeNode.uri);
    } else if (treeNode && !treeNode.noR) {
        showRMenu("node", event.clientX, event.clientY,treeNode.id,treeNode.name,treeNode.uri);
    }
}
//显示右键菜单
function showRMenu(type, x, y,id,title,url) {
    $("#rMenu").show();
...
function addMenu()
{
    ...
    if(url==null || ""==url)
    {
        $.alle_dialog("/infomanager/usermanager/menu_add.html?menu_id="+menuId+"&role_id="+role_id, title, ["300px", "65%"]);
    }else {
        layer.msg("无法添加");
    }
