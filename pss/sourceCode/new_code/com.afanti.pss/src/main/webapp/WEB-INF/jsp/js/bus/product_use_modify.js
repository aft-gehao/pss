...
function doSearch(use_id) {
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function (data) {
                        $("#use_name").val(data["append"]["use_name"]);
                        $("#use_time").val($.alle_time2str_yymm_dd_hhmmss(data["append"]["use_time"]));
                        $("#amount").val(data["append"]["use_amount"]);
                        $("#unit").html(data["append"]["use_unit"]);
                        $("#nameEn").val(data["append"]["nameEn"]);
...
*/$("#submit").click(function(){
       ...
       var use_name=$("#use_name").val();
       var use_amount=$("#amount").val();
       var use_id = $.alle_getUrlParam("use_id");
    if(!use_amount)
    {
        tip_msg("请填写出库量", "#amount");
    }
        $.ajax({
            type: 'POST',
            url: "/product_use/manage/useModify",
            data: {
                use_name: use_name,
